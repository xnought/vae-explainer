...
<svg {x} {y} {width} {height} style="overflow: visible;">
  ...
  <!-- e^{0,5*log(var)} -->
  <TwoFunc x={firstExpX} y={firstExpY} f={halfExp} domain={[-10, 0]} range={[-0.1, 1]} inputs={logVar} color="lightseagreen" tex={String.raw`e^{x/2}`} opacity={ho($hoveringlogVarTrick)}/>
